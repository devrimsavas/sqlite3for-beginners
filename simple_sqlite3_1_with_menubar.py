...
def about_program():
    ...
    You can add search and quit function. Feel free to change it.
    Devrim Savas Yilmaz"""
...
def styles():
    style_menu = Toplevel(root)
    style_menu.title("Styles")
    style_menu.geometry("200x200")

    for style_name in style_list:
        Button(style_menu, text=style_name, command=lambda style=style_name: change_style(style)).pack()
...
stylemenu.add_command(label="Styles", command=styles)
